...
const styles = {
  cardCategoryWhite: {  // Style for card category text when background is dark
    "&,& a,& a:hover,& a:focus": {  // Targets element and nested anchor tags in all states
    ...
    "& a,& a:hover,& a:focus": {  // Targets anchor tags in all states
      color: "#FFFFFF",  // Full white text for links
    },
  },
  cardTitleWhite: {  // Style for card title text when background is dark
...
export default function LabReports() {
  ...
  const getAllTests = () => {
    fetch(`${base}/KNH/patient/lab/tests/report`)  // API call to get all lab test reports
      ...
      .then((data) => {
          if (data.message == "Report Found") {  // If reports were found
            ...
            <div className="repContainer">  {/* Container for report content */}
              ...
              <div className="reportBody">
                ...
                <div>
                  ...
                  {rows.length > 0 ? 
                  <table className="styled-table">  {/* Laboratory tests table */}
                    ...
                    <tbody>  {/* Table body */}
                      ...
                      {rows.length > 0 ? rows.map((item) => (
                        <tr>
                          <td className="trBody">{item.lab_test_date}</td>  {/* Test date */}
                          <td className="trBody">{item.test_name}</td>  {/* Test name */}
                          <td className="trBody">{item.patient_id}</td>  {/* Patient ID */}
                          <td className="trBody">{item.test_results}</td>  {/* Test results */}
                          <td className="trBody">Ksh {item.test_cost}</td>  {/* Test cost with currency */}
                        </tr>
                      ))
                      :
              ...
              {/* CSV export button */}
              <div className="print">
                  <CSVLink data={rows} className="excel">Excel</CSVLink>  {/* Link to download report as CSV */}
              </div>
              </>
              : null }  {/* Render nothing if no rows */}
            </div>
